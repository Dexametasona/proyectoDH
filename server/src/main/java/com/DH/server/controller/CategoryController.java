...
public class CategoryController {
    ...
    @PostMapping
    public ResponseEntity<?> createCategory(@ModelAttribute
    ...
    @Operation(summary = "Get Category",description = "Get all categories with pagination")
    @GetMapping
    public ResponseEntity<?> getAll(
            @Parameter(description = "Pagination and sorting")
            @Nullable Pageable page){
        Page<Category> categories =this.categoryService.getAll(page);
        Page<CategoryResDto> categoryResDtos=categories.map(categoryMapper::toResponse);
        return ResponseEntity.ok(
                new ApiResponseDto<>(this.categoryMapper.toCustomPage(categoryResDtos)));
    }
